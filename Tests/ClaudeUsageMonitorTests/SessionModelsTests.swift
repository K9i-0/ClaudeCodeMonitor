...
final class SessionModelsTests: XCTestCase {
    ...
    func testSessionUsagePercentage() {
        ...
        usageData.activeSession = createMockSession(totalTokens: 3_000)
    ...
    func testTokenCountsBillableTokens() {
        ...
        XCTAssertEqual(tokenCounts.billableTokens, 1_500)
    }

    func testSessionBlockConversion() {
        let session = createMockSession(totalTokens: 5_000)
    ...
    private func createMockSession(totalTokens: Int) -> SessionBlock {
        ...
        return SessionBlock(
            id: "test-session",
            startTime: now.addingTimeInterval(-3_600).ISO8601Format(),
            endTime: now.addingTimeInterval(14_400).ISO8601Format(),
            actualEndTime: nil,
            isActive: true,
            isGap: false,
            entries: 10,
